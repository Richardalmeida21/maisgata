...
{% set show_help = not (has_main_slider or has_mobile_slider or has_video or has_main_categories or has_banners or has_promotional_banners or has_news_banners or has_image_and_text_module or has_brands or has_informative_banners or has_instafeed or has_testimonials or has_welcome_message or has_institutional_message) and not has_products %}
...
<div class="js-home-sections-container">
    ...
    {% if show_component_help %}
        <div style="display:none">
            {% for section_select in ['slider', 'main_categories', 'welcome', 'products', 'informatives', 'categories', 'main_product', 'new', 'video', 'sale', 'instafeed', 'promotional', 'news_banners', 'brands' , 'newsletter' , 'testimonials', 'institutional', 'modules'] %}
                {% if section_select not in newArray %}
                    {% include 'snipplets/home/home-section-switch.tpl' %}
                {% endif %}
            {% endfor %}
