    ...
    {% if has_applied_filters %}
        ...
        {% for product_filter in product_filters %}
            {% for value in product_filter.values %}
                ...
                {% if value.selected %}
                    <button class="js-remove-filter chip {{ chip_class }}" data-filter-name="{{ product_filter.key }}" data-filter-value="{{ value.name }}" data-component="filter.pill-{{ product_filter.type }}" data-component-value="{{ product_filter.key }}">
                        {{ value.pill_label }}
        ...
        <div id="filters" class="visible-when-content-ready" data-store="filters-nav">
            ...
            {% for product_filter in product_filters %}
                {% if product_filter.type == 'price' %}
                    ...
                    {{ component(
                        'price-filter',
                        {'group_class': 'price-filter-container pb-1', 'title_class': 'h5 mb-3 mt-4', 'button_class': 'btn btn-secondary btn-small ' ~  btn_price_classes }
                    ) }}
