...
<div class="price-container mb-4" data-store="product-price-{{ product.id }}">
  <div class="mb-2">
    ...
    <span class="d-inline-block">
      <div
        ...
        class="js-compare-price-display price-compare h3"
    ...
    {{
      ...
      location: 'product',
      container_classes: "h6 mt-1 text-accent",
      text_classes: {
      price: 'h3',
      },
      })
  ...
  {% if not product.is_non_shippable and show_product_quantity and (has_free_shipping or has_product_free_shipping) %}
    <div class="free-shipping-message mt-2 mb-4">
      <span class="text-accent">{{ 'Envío gratis' | translate }}</span>
      <span
        {% if has_product_free_shipping %}
          style="display: none;"
        {% else %}
          class="js-shipping-minimum-label"
        {% endif %}
      >
        {{ 'superando los' | translate }}
        <span>{{ cart.free_shipping.min_price_free_shipping.min_price }}</span>
      </span>
...
{{
  ...
  promotion_title: 'mb-2 h6 text-accent',
...
>
  ...
  {% if settings.last_product and show_product_quantity %}
    <div
      ...
      {% if product.selected_or_first_available_variant.stock != 1 %}
        style="display: none;"
      {% endif %}
    >
  ...
  <div class="form-row mb-4">
    ...
    <div class="{% if show_product_quantity %}col-8{% else %}col-12{% endif %}">
      ...
      <input
        type="submit"
        class="js-addtocart js-prod-submit-form btn btn-primary btn-block mb-4 {{ state }}"
      ...
      <div class="col-12">
        <div class="js-added-to-cart-product-message font-small mb-3 pb-2" style="display: none;">
          ...
          <span>
            ...
            <a
              href="#"
              class="js-modal-open js-open-cart js-fullscreen-modal-open btn-link float-right ml-1"
    ...
    {% if not product.is_non_shippable and show_product_quantity and has_free_shipping and not has_product_free_shipping %}
      ...
      <div
        class="{% if free_shipping_minimum_label_changes_visibility %}js-free-shipping-message{% endif %} mt-2 text-accent"
        {% if not cart.free_shipping.cart_has_free_shipping %}
          style="display: none;"
        {% endif %}
      >
  ...
  {% if template == 'product' %}
    <div class="row m-md-0">
          ...
          <div class="js-accordion-content pt-3" style="display: none;">
            <div
              {% if installments_info %}
                data-toggle="#installments-modal" data-modal-url="modal-fullscreen-payments"
              {% endif %}
              class="{% if installments_info %}js-modal-open js-fullscreen-modal-open{% endif %} js-product-payments-container row mb-4"
              {% if not (product.get_max_installments and product.get_max_installments(false)) %}
                style="display: none;"
              {% endif %}
            >
              ...
              <span
                class="js-product-discount-container col-12 mb-2"
                {% if hideDiscountContainer %}
                  style="display: none;"
                {% endif %}
              >
                <span class="text-accent">
                  {{- product.maxPaymentDiscount.value }}% {{ 'de descuento' | translate -}}
                </span>
                {{ 'pagando con' | translate }}
                {{ product.maxPaymentDiscount.paymentProviderName }}
                <div
                  class="js-product-discount-disclaimer font-small mt-1"
                  {% if hideDiscountDisclaimer %}
                    style="display: none;"
                  {% endif %}
                >
                  {{ 'No acumulable con otras promociones' | translate }}
              ...
              <a
                id="btn-installments"
                class="btn-link font-small col mt-1"
                {% if not (product.get_max_installments and product.get_max_installments(false)) %}
                  style="display: none;"
                {% endif %}
              >
      ...
      {% if show_product_fulfillment %}
        ...
        <div
          id="product-shipping-container"
          class="product-shipping-calculator list accordion px-3 col-12"
          {% if not product.display_price or not product.has_stock %}
            style="display:none;"
          {% endif %}
          data-shipping-url="{{ store.shipping_calculator_url }}"
        >
